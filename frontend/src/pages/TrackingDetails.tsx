            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        {/* AI Smart Tracking Insights */}
                        <div className="bg-gradient-to-br from-[#111F35] to-[#2A1B3D] border-2 border-[#8A244B] rounded-xl p-6 shadow-lg relative overflow-hidden group">
                            <div className="absolute top-0 right-0 w-32 h-32 bg-[#F63049] opacity-5 rounded-full blur-3xl transform translate-x-10 -translate-y-10 group-hover:opacity-10 transition-opacity duration-500"></div>

                            <div className="flex items-center mb-6">
                                <div className="bg-[#F63049] bg-opacity-20 p-3 rounded-lg mr-4">
                                    <svg className="w-8 h-8 text-[#F63049]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div>
                                    <h2 className="text-2xl font-bold text-white flex items-center">
                                        AI Smart Insights
                                        <span className="ml-3 px-2 py-1 bg-[#F63049] text-xs font-bold rounded uppercase tracking-wider text-white">BETA</span>
                                    </h2>
                                    <p className="text-gray-400 text-sm">Powered by Predictive Logistics AI</p>
                                </div>
                            </div>

                            <div className="grid md:grid-cols-2 gap-6">
                                <div className="space-y-4">
                                    <div className="bg-[#111F35] bg-opacity-50 p-4 rounded-lg border border-[#8A244B]">
                                        <p className="text-gray-400 text-sm mb-1 uppercase tracking-wider">Next Predicted Location</p>
                                        <div className="flex items-center text-[#F63049] font-semibold">
                                            <svg className="w-5 h-5 mr-2 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                                            </svg>
                                            {trackingData?.aiInsights?.predictedNextLocation || 'Calculating...'}
                                        </div>
                                    </div>

                                    <div className="bg-[#111F35] bg-opacity-50 p-4 rounded-lg border border-[#8A244B]">
                                        <p className="text-gray-400 text-sm mb-1 uppercase tracking-wider">AI Predicted Delivery</p>
                                        <div className="flex items-center text-white font-semibold">
                                            ...
                                            {trackingData?.aiInsights?.predictedDeliveryDate || trackingData?.eta?.date}
                                        </div>
                                    </div>
                                </div>

                                <div className="space-y-4">
                                    {trackingData?.aiInsights?.abnormalStops?.length > 0 ? (
                                        <div className="bg-red-900 bg-opacity-20 p-4 rounded-lg border border-red-500/50">
                                            <div className="flex items-center text-red-400 mb-2 font-semibold">
                                                <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                                                </svg>
                                                Abnormal Stops Detected
                                            </div>
                                            <ul className="list-disc list-inside text-sm text-gray-300 space-y-1 ml-1">
                                                {trackingData.aiInsights.abnormalStops.map((stop: string, idx: number) => (
                                                    <li key={idx}>{stop}</li>
                                                ))}
                                            </ul>
                                        </div>
                                    ) : (
                                        <div className="bg-green-900 bg-opacity-20 p-4 rounded-lg border border-green-500/50 flex items-center justify-center h-full">
                                            <div className="text-center">
                                                <svg className="w-8 h-8 text-green-400 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                                </svg>
                                                <span className="text-green-400 font-medium">No Abnormal Stops</span>
                                            </div>
                                        </div>
                                    )}

                                    {trackingData?.aiInsights?.suggestedActions?.length > 0 && (
                                        <div className="bg-[#111F35] bg-opacity-50 p-4 rounded-lg border border-[#8A244B]">
                                            <p className="text-gray-400 text-sm mb-2 uppercase tracking-wider">Suggested Actions</p>
                                            <ul className="space-y-2">
                                                {trackingData.aiInsights.suggestedActions.map((action: string, idx: number) => (
                                                    <li key={idx} className="flex items-center text-sm text-gray-300">
                                                        <svg className="w-4 h-4 text-[#F63049] mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                            ...
                            <div className="bg-gradient-to-br from-[#111F35] to-[#8A244B] border border-[#8A244B] rounded-xl p-6 shadow-lg">
                                ...
                                <p className="text-sm text-gray-400 mt-3">
                                    Confidence: <span className={`font-semibold ${trackingData?.eta?.confidence > 80 ? 'text-green-400' : 'text-yellow-400'}`}>{trackingData?.eta?.confidence}%</span>
                                </p>
                            </div>
